...
export function formatTerminal(scorecard: TierScorecard): void {
  ...
  const rows: [string, MatrixRow, CheckStatus | null][] = [
    ['Server', matrix.server, c.conformance.status],
    ['Client: Core', matrix.clientCore, null],
    ['Client: Auth', matrix.clientAuth, null]
  ];

  for (const [label, row, status] of rows) {
    const icon = status ? statusIcon(status) + ' ' : '  ';
    console.log(
      `  ${icon}${lp(label, lw)} ${SPEC_VERSIONS.map((v) => rp(formatCell(row.cells.get(v)), vw)).join(' ')}  ${rp(formatRate(row.unique), tw)}`
    );
  }
