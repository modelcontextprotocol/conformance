 ...
 */
const SECURE_SIGNING_ALGORITHMS = ['ES256', 'ES384', 'ES512', 'RS256', 'RS384', 'RS512', 'PS256', 'PS384', 'PS512'];
...
- RFC 7523 (JWT Client Authentication)`;
  ...
  async run(serverUrl: string): Promise<ConformanceCheck[]> {
    ...
    const signingAlgorithms = metadata.token_endpoint_auth_signing_alg_values_supported;
    ...
    if (authMethods === undefined) {
      checks.push({
        ...
        description: 'AS metadata contains token_endpoint_auth_methods_supported field',
        status: 'INFO',
        timestamp: timestamp(),
        errorMessage: 'Field not present - defaults to ["client_secret_basic"] per RFC 8414',
    ...
    if (authMethods.length === 0) {
      checks.push({
        ...
        status: 'FAILURE',
        timestamp: timestamp(),
        errorMessage: 'Empty array - at least one auth method must be supported',
        specReferences: [ServerAuthSpecReferences.RFC_8414_AS_FIELDS],
        details: { token_endpoint_auth_methods_supported: authMethods }
      });
      return checks;
    ...
    const invalidMethods = authMethods.filter((m: unknown) => typeof m !== 'string' || !VALID_AUTH_METHODS.includes(m as string));
    ...
    if (invalidMethods.length > 0) {
      checks.push({
        ...
        specReferences: [ServerAuthSpecReferences.OAUTH_2_1_TOKEN_ENDPOINT_AUTH],
        details: {
    ...
    if (authMethods.length === 1 && authMethods[0] === 'none') {
      checks.push({
        ...
        description: 'Check if only public clients (no authentication) supported',
        status: 'INFO',
        timestamp: timestamp(),
        specReferences: [ServerAuthSpecReferences.OAUTH_2_1_TOKEN_ENDPOINT_AUTH],
        details: {
    ...
    if (hasPrivateKeyJwt || hasClientSecretJwt) {
      ...
      if (signingAlgorithms === undefined) {
        checks.push({
          ...
          description: 'Check token_endpoint_auth_signing_alg_values_supported for JWT auth',
          status: 'WARNING',
          timestamp: timestamp(),
          errorMessage: 'JWT auth supported but token_endpoint_auth_signing_alg_values_supported not advertised',
          specReferences: [
            ServerAuthSpecReferences.RFC_7523_JWT_CLIENT_AUTH,
            ServerAuthSpecReferences.RFC_8414_AS_FIELDS
          ],
          details: {
            jwt_auth_methods: authMethods.filter((m: unknown) => m === 'private_key_jwt' || m === 'client_secret_jwt'),
        ...
        checks.push({
          ...
          specReferences: [ServerAuthSpecReferences.RFC_8414_AS_FIELDS],
          details: { token_endpoint_auth_signing_alg_values_supported: signingAlgorithms }
        });
      } else {
        // Check for secure algorithms
        const secureAlgs = signingAlgorithms.filter((alg: unknown) => typeof alg === 'string' && SECURE_SIGNING_ALGORITHMS.includes(alg as string));
        ...
        if (secureAlgs.length > 0) {
          ...
          checks.push({
            ...
            status: 'WARNING',
            timestamp: timestamp(),
            errorMessage: 'No secure signing algorithms found (ES256, RS256, etc. recommended)',
    ...
    if (hasClientSecretBasic) {
      checks.push({
        ...
        description: 'Authorization Server supports client_secret_basic authentication',
        status: 'SUCCESS',
        timestamp: timestamp(),
        specReferences: [ServerAuthSpecReferences.OAUTH_2_1_TOKEN_ENDPOINT_AUTH],
        details: {
    ...
    checks.push({
      ...
      description: 'Authorization Server supports authentication for confidential clients',
